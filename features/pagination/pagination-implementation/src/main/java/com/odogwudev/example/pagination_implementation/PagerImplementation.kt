package com.odogwudev.example.pagination_implementation

import com.odogwudev.example.pagination_api.*
...
class PagerImplementation : Pager {
    ...
    override suspend fun paginate(
        ...
        RefreshType.CACHE_IF_POSSIBLE -> flow.value.ifEmpty {
            if (cacheWithError) {
                paginationResult(function = {
                    getRemoteContent(1).also {
                        flow.value = it.appendTailItem()
                    }
                }, fallbackContent = emptyList(), flow = flow, sideEffect = { COUNTER = 2 })
            } else {
                getRemoteContent(1).also {
                    flow.value = it.appendTailItem()
                }
        ...
        RefreshType.NEXT_PAGE -> {
            ...
            }, fallbackContent = oldContent, flow = flow, sideEffect = { COUNTER++ })
        }
        RefreshType.FORCE_REFRESH -> paginationResult(function = {
            getRemoteContent(1).also {
                flow.value = it.appendTailItem()
            }
        }, fallbackContent = flow.value, flow = flow, sideEffect = { COUNTER = 2 })
    }
    ...
    private inline fun paginationResult(
        ...
        flow.value = fallbackContent.filter { it !is ErrorItem && it !is TailItem } + listOf(
            ErrorItem(errorMessage = exception.message.orEmpty())
        )
        throw exception
    }

    private fun List<PagerItem>.appendTailItem() =
        this + listOf(TailItem(loadMore = this.isNotEmpty()))
